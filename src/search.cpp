...
void iterate(const int side, const int depth)
{
	...
	const auto node_count = static_cast<bitboard*>(malloc(MAX_MOVES * sizeof(bitboard)));
	...
	const auto move_list = reinterpret_cast<unsigned int*>(&ply_info[0].move_list);
		...
		if (score != -MAX)
		{
			...
			for (i = 0; i < num; i++)
				for (int j = i + 1; j < num; j++)
					...
					{
						const unsigned int temp = move_list[i];
						move_list[i] = move_list[j];
						move_list[j] = temp;
						const bitboard temp_count = node_count[i];
			...
			if (score > NEAR_MATE)
			{
				...
				if (secs > 1)
				{
					...
					nodes_per_check = static_cast<int>(nodes_per_check / secs);
				}
				free(node_count);
				return;
			}
...
int search(const int ply, const int depth, const int side, int alpha, int beta, const int null_move)
{
	nodes++;
	...
	int num = 0;
	unsigned int move;
	const int small_ext = ply > initial_depth << 1;
...
int root_search(const int side, const int depth, const unsigned int* move_list, bitboard* node_count, const int num_moves, int alpha, const int beta)
{
	pv_length[0] = 0;
	int value;
...
int quiescient(const int ply, const int side, int alpha, const int beta)
{
	nodes++;
	pv_length[ply] = ply;
	...
	const int h = eval(side, alpha, beta);
	...
	const int delta = alpha - pawn_value - side * board->score;
	...
	for (int k = 0; k < num; k++)
		for (int curr_depth = k + 1; curr_depth < num; curr_depth++)
			if (sort[k] > sort[curr_depth])
			{
				const unsigned int temp = ply_info[ply].move_list[k];
