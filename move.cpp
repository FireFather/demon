...
void make_move(const unsigned int move, const int ply)
{
	...
	bitboard temp = mask[from] | mask[to];
...
void un_make_move(const unsigned int move, const int ply)
{
	const int piece = PIECE(move);
	const int from = FROM(move);
	const int to = TO(move);
...
unsigned int next_move(const int ply, const int side)
{
